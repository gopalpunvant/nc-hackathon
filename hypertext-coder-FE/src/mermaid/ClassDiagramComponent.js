...
const ClassDiagramComponent = () => {
    ...
    useEffect(() => {
        const generatedMermaidText = generateMermaidDiagram(design);
    ...
    const generateMermaidDiagram = (jsonData) => {
        ...
        jsonData.forEach((classInfo) => {
            ...
            classInfo.properties.forEach((prop) => (mermaidText += `  ${classInfo.name} : ${prop}\n`));
            classInfo.methods.forEach((method) => (mermaidText += `  ${classInfo.name} : ${method}\n`));
        });
